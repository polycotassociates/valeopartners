 ...
 * Class MailHandler.
 *
 * From the commerce project. Thanks to @bojanz.
 *
 * @package Drupal\commerce_product_reminder
 */
class MailHandler implements MailHandlerInterface {
  ...
  public function sendMail($to, $subject, array $body, array $params = []) {
    ...
    $default_params = [
      ...
      // The body will be rendered in entity_activity_mail_mail(), because
      // that's what MailManager expects. The correct theme and render context
      // aren't setup until then.
      'body' => $body,
    ];
    if (!empty($params['bcc'])) {
    ...
    $message = $this->mailManager->mail('entity_activity_mail', $params['id'], $to, $params['langcode'], $params);
